   ...
   <div class="filters">
      <div class="filters__head">
         <div class="filters__mode">
            <span :class="mapClass" @click="$emit('clickMap')">{{ lang.map }}</span>
            <span :class="listClass" @click="$emit('clickList')">{{ lang.list }}</span>
         </div>
         <div class="filters__city">
            <city-filter
                    ...
                    @change-filter-region="regionsChange"
                    @change-filter-city="cityChange">
            </city-filter>
         </div>
         <show-button
            ...
            <div class="filters__dropdown-wrapper">
               <div class="filters__group">
                  <work-filter
                        ...
                        :filters-changed="filtersChangeState"
                        :lang="lang"
                        @clear-filters="updateFiltersState(true)"
               ...
               <div class="filters__button-wrapper">
                  <button class="clear-filters for-mobile" @click="$emit('is-open:filters')" >{{ lang.submit_filters }}</button>
               </div>
            </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      regionsData() {
        return JSON.parse(this.jsonFiltersData).regions;
      },
      mapClass() {
        return !this.isOpenDepartments ? 'active' : '';
      },
      listClass() {
        return this.isOpenDepartments ? 'active' : '';
      },
    ...
    methods: {
      ...
      // region change event
      regionsChange(region) {
        this.filterConfig.region = region ? region.value : '';
        this.filtration();
      },
      cityChange(city) {
        this.filterConfig.city = city ? city.value : '';
        this.filtration();
      },
      ...
      filtration() {
        const { days, reception, timeline, region, city } = this.filterConfig;
        ...
        this.departments.forEach((department) => {
          ...
          this.filtrationRegion(department, isPassTest, region);
          this.filtrationCity(department, isPassTest, city);
      ...
      // filter by city
      filtrationCity(department, statesObject = {}, city) {
        const states = statesObject;

        if (!city) {
          states.city = true;
        } else if (department.city) {
          states.city = (department.city === city);
        } else {
          states.city = false;
        }
      },
      // filter by region
      filtrationRegion(department, statesObject = {}, region) {
        const states = statesObject;

        if (!region) {
          states.region = true;
        } else if (department.region) {
          states.region = (department.region === region);
        } else {
          states.region = false;
        }
      },
