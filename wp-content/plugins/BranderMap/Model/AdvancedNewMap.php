 ...
 */
class AdvancedNewMap extends MapAbstract
{
    ...
    public function createMap()
    {
        ...
        try {
            ...
            $path = $this->_config->getTemplatesPath() . '/advanced_new_map-template.php';
    ...
    protected function _setFilters()
    {
        if ($this->_params['filters'] && !empty($this->_params['filters'])) {
            ...
            $this->_setRegionFiltersData();
        }
    }

    protected function _setRegionFiltersData()
    {
        $regionFilters = [];
        $cityFilters = [];
        if (!empty($this->_mapData)) {
            foreach ($this->_mapData as $department) {
                if ($department) {
                    $departmentFilters = wp_get_post_terms($department->ID, 'regions_cities');

                    if (!empty($departmentFilters)) {
                        foreach ($departmentFilters as $filter) {
                            if ($filter->parent != 0) {
                                $regionFilters[$filter->parent]['label'] = get_term($filter->parent, 'regions_cities')->name;
                                $regionFilters[$filter->parent]['value'] = $filter->parent;
                                $regionFilters[$filter->parent]['cities'][$filter->term_id]['label'] = $filter->name;
                                $regionFilters[$filter->parent]['cities'][$filter->term_id]['value'] = $filter->term_id;
                            }
                        }
                    }
                }
        ...
        $this->filtersData['regions'] = $regionFilters;
    }
    ...
    protected function _setReceptionFiltersData()
    {
                    ...
                    if (!empty($departmentFilters)) {
                        foreach ($departmentFilters as $filter) {
                            if (!isset($receptionFilters[$filter->term_id])) {
                                $image = get_field('image', 'reception_'.$filter->term_id);
                                $receptionFilters[$filter->term_id] = $filter->name;
                                $receptionImages[$filter->term_id] = $image['url'];
                            }
                        }
                    }
                }
        ...
        $this->filtersData['image'] = $receptionImages;
    }
